...
def render_box_and_scatter_chart(ax: Axes, df: pd.DataFrame, sheet_name: str = "Data") -> None:
    # 1. 准备x轴的标签
    x_labels = []
    ...
    for i, (label, values) in enumerate(combined_data.items()):
        ...
        ax.scatter(x_jittered, values,
                   color=scatter_color, alpha=0.6, s=40,
    ...
    for i, (label, values) in enumerate(combined_data.items()):
        ...
        ax.barh(bin_edges[:-1], hist, height=bin_width,
    ...
    # 8. 图例配置
    legend_elements = [
        Line2D([0], [0], marker='o', color='w', label='Data Points', markerfacecolor='gray', markersize=10),
